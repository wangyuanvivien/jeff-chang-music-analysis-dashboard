...
def load_data():
    ...
    if not os.path.exists(DATA_FILE_A):
        ...
        st.info("請確認您已成功運行 'merge_ultimate_master.py' 腳本。")
        return None
        ...
        try:
            ...
            if all(col in df_ai.columns for col in AI_COLS):
                ...
                for key in ['track_name', 'album_title']:
                    df_ultimate[key] = df_ultimate[key].astype(str).fillna('__MISSING__')
                    df_ai[key] = df_ai[key].astype(str).fillna('__MISSING__')
...
# --- 3b. 成功的資料載入 ---
    ...
    selected_song = st.sidebar.selectbox(
        ...
        options=song_list,
        index=None,  # 預設為空 (不選擇)
        placeholder="瀏覽所有歌曲 (Browse All Songs)"
    )
    ...
    if selected_song is None:
        ...
        if st.session_state.get('ai_available', False) and 'ai_theme' in df.columns:
            songs_with_ai = df['ai_theme'].notna().sum()
        ...
        # --- 用於繪圖的輔助函式 ---
        @st.cache_data
        def plot_categorical_chart(df, column, title):
            """ 繪製分類型別的長條圖 """
            # 確保欄位存在且有資料
            ...
            # 計算 value counts
            chart_data = data[column].value_counts().head(15).reset_index() # 最多顯示前 15
            ...
            chart = alt.Chart(chart_data).mark_bar().encode(
                ...
                title=f"{title} 分佈 (前 15)"
            ).interactive() # 允許縮放和拖動
            st.altair_chart(chart, use_container_width=True)

        @st.cache_data
        def plot_histogram(df, column, title):
            """ 繪製數值型別的直S_histogram (Histogram) """
            if column not in df.columns or df[column].dropna().empty:
                st.caption(f"欄位 '{column}' 無足夠資料可供繪圖。")
                return
            ...
            chart = alt.Chart(data).mark_bar().encode(
                alt.X(column, bin=True, title=title), # bin=True 會自動建立直方圖
                alt.Y('count()', title='歌曲數量 (Count)'),
                tooltip=[alt.X(column, bin=True, title=title), 'count()']
            ).properties(
        ...
        with chart_col1:
            # 您要求的 'genre_ros'
            plot_categorical_chart(df, 'genre_ros', '音樂流派 (Genre)')
        
        with chart_col2:
            plot_categorical_chart(df, 'key_scale', '音樂調式 (大/小調)')
        
        with chart_col3:
            plot_categorical_chart(df, 'key_key', '歌曲調性 (Key)')
        ...
        with chart_col4:
            # 您要求的 'mood_party'
            plot_histogram(df, 'mood_party', '派對指數 (Mood: Party)')
            
        with chart_col5:
            plot_histogram(df, 'danceability', '舞蹈指數 (Danceability)')
